...
void Main(void) {
    ...
    /** Creating Scene */
    T3_Entity *main;
    T3_Component *scene = T3C_Scene_Init("Main Scene", 1024, 768);
    
    T3_Entity *mainCamera;
    T3_Component *camera = T3C_Camera_Init();
    T3_Component *positionCamera = T3C_Position_Init(0, 0);
    
    T3_Entity *exampleSprite;
    ...
    T3_Component *spriteRenderer = T3C_SpriteRenderer_Init_With_Camera(camera->Data);
