...
T3_Node *T3_Node_Init(void *data) {
    T3_Node *node = T3_Helper_Malloc_Safe(sizeof *node, T3_FILE, T3_LINE);
    node->Data = data;
    node->Next = NULL;
    return node;
}
...
T3_NodeDouble *T3_NodeDouble_Init(void *data) {
    T3_NodeDouble *nodeDouble = T3_Helper_Malloc_Safe(sizeof *nodeDouble, T3_FILE, T3_LINE);
...
T3_Node *T3_Node_Init_Int(int data) {
    int *value = T3_Helper_Malloc_Safe(sizeof *value, T3_FILE, T3_LINE);
    *value = data;
    return T3_Node_Init(value);
}

T3_Node *T3_Node_Init_Float(float data) {
    float *value = T3_Helper_Malloc_Safe(sizeof *value, T3_FILE, T3_LINE);
    *value = data;
    return T3_Node_Init(value);
}
